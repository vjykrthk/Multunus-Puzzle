...
exports.retweeters = function (req, res) {
    ...
    async.waterfall([
        ...
        function(updated_time, cb) {            
            ...
            twit.get('statuses/user_timeline', { user_id: tweeter_id, count: 10, include_rts: false, exclude_replies: true }, function (err, reply) {
                ...
                if(diff > 15) {
                    ...
                    function(err, retweeters) {
                        if(err) {
                            ...
                            var r = twitter.get_filtered_retweeters(retweeters);

                            async.parallel([
                            function() {
                                res.json({ tweeter: tweeter, retweeters: r });
                            }, 
                            function() {
                                database.set_tweeter_updatedtime(tweeter_id);                            
                                database.insert_retweeters(tweeter_id, r);
                            }]);
                        }
                    });
                } else {
                    ...
                    function (retweeters, cb) {
                        res.json({ tweeter: tweeter, retweeters: retweeters });
                    }]);
                }              
            });   
        }
